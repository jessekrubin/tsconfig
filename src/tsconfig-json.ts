...
export type TsconfigCompilerOptions = {
   ...
   * 'ES6', 'ES2015' or 'ESNext'. Only 'AMD' and 'System' can be used in
   * conjunction with `--outFile`. 'ES6' and 'ES2015' values may be used when
   * targeting 'ES5' or lower.
   *
   * Default: `'ESNext'` since TypeScript 6.0, `['ES3', 'ES5'].includes(target)
   * ? 'CommonJS' : 'ES6'` before.
   */
  ...
  /**
   * Allows TypeScript files to import each other with a TypeScript-specific
  ...
  /**
   * Forces TypeScript to consult the exports field of package.json files if it
  ...
  /**
   * Forces TypeScript to consult the imports field of package.json files when
   * performing a lookup that starts with # from a file whose ancestor directory
  ...
  /**
   * List of additional conditions that should succeed when TypeScript resolves
   * from package.json.
   */
...
export type TsconfigTypeAcquisition = {
  ...
  /**
   * Disable infering what types should be added based on filenames in a
   * project.
   */
  disableFilenameBasedTypeAcquisition?: boolean;
};

export type TsconfigReferences = {
  ...
  /**
   * True if the output of this reference should be prepended to the output of
   * this project.
   *
...
export type TsConfigJson = {
  ...
  /** Auto type (.d.ts) acquisition options for this project. */
  typeAcquisition?: TsconfigTypeAcquisition;
   ...
   * If no `files` or `include` property is present in a `tsconfig.json`, the
   * compiler defaults to including all files in the containing directory and
   * subdirectories except those specified by `exclude`.
   */
  include?: string[];
